    ...
    workbook.csv.readFile(filename)
        .then(function(worksheet) {
            worksheet.eachRow({
                ...
                var data = "insert into Productos values (" + rowNumber + ", '" +
                    row.getCell(1).value.trim() + "', '" + row.getCell(1).value.trim() + "', '" + row.getCell(1).value.trim() + "', '" +
                    row.getCell(1).value.trim() + " DESC" + "', '" + imagen64 + "', 1);"
                inserts.push(data);
